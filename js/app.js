...
document.querySelectorAll(".tabs button").forEach((b) => {
  b.addEventListener("click", () => {
    ...
    for (const t of ["saju", "fortune", "name", "gwansang", "palm", "learn", "about"])
...
// ── 손금 ─────────────────────────────────────────────────
let palmCam = null;
$("#palm-upload").addEventListener("click", () => $("#palm-file").click());
$("#palm-file").addEventListener("change", (e) => {
  const f = e.target.files[0];
  if (!f) return;
  const img = new Image();
  img.onload = () => { palmDraw(img); palmAnalyze(); };
  img.src = URL.createObjectURL(f);
});
$("#palm-camera").addEventListener("click", async () => {
  try {
    // 손바닥은 후면 카메라가 편하다 (없으면 전면)
    palmCam = await navigator.mediaDevices.getUserMedia({ video: { facingMode: "environment", width: 1080 } })
      .catch(() => navigator.mediaDevices.getUserMedia({ video: { facingMode: "user", width: 1080 } }));
    const v = $("#palm-cam");
    v.srcObject = palmCam;
    v.classList.remove("hidden");
    $("#palm-shoot").classList.remove("hidden");
    $("#palm-status").textContent = "손바닥을 화면 가득 채우고 [촬영]을 눌러 주세요.";
  } catch (err) {
    $("#palm-status").textContent = "카메라를 열 수 없습니다 (" + err.name + "). 사진 올리기를 이용해 주세요.";
  }
});
$("#palm-shoot").addEventListener("click", () => {
  ...
  $("#palm-shoot").classList.add("hidden");
  palmAnalyze();
});
$("#palm-clear").addEventListener("click", () => {
  if (palmCam) { palmCam.getTracks().forEach((t) => t.stop()); palmCam = null; }
  $("#palm-cam").classList.add("hidden");
  $("#palm-shoot").classList.add("hidden");
  const cv = $("#palm-canvas");
  cv.getContext("2d").clearRect(0, 0, cv.width, cv.height);
  cv.classList.add("hidden");
  $("#palm-views").classList.add("hidden");
  $("#palm-clear").classList.add("hidden");
  $("#palm-file").value = "";
  PALM_PHOTO.base = PALM_PHOTO.views = null;
  $("#palm-status").textContent = "지웠습니다. 사진은 어디에도 남지 않습니다.";
});
function palmDraw(src, w, h) {
  const cv = $("#palm-canvas");
  const sw = w || src.naturalWidth, sh = h || src.naturalHeight;
  const scale = Math.min(1, 760 / sw);
  cv.width = Math.round(sw * scale); cv.height = Math.round(sh * scale);
  cv.getContext("2d").drawImage(src, 0, 0, cv.width, cv.height);
  cv.classList.remove("hidden");
  $("#palm-clear").classList.remove("hidden");
}
...
document.querySelectorAll(".vtog").forEach((b) => {
  b.addEventListener("click", () => {
    document.querySelectorAll(".vtog").forEach((x) => x.classList.toggle("on", x === b));
    PALM_PHOTO.show($("#palm-canvas"), b.dataset.v);
  });
});
...
function renderPalmAutoResult(auto) {
  ...
  // 닫는 카드
  const c3 = el("div", "card");
  c3.appendChild(el("h2", null, "마지막으로 드리고 싶은 말 🌱"));
  c3.appendChild(el("div", "rbody",
    `<p>${PALM_DB.closing}</p>` +
...
function renderLearn() {
  ...
  const steps = [
    ...
    ["오행은 물질이 아니라 다섯 가지 '움직임'입니다",
      `옛사람들이 계절이 도는 모습에서 뽑아낸 다섯 에너지 패턴입니다.<br>` +
      `🌱 <b>목</b> — 뻗어 오르는 힘 (봄 · 시작 · 호기심)<br>` +
      `🔥 <b>화</b> — 퍼져 나가는 힘 (여름 · 열정 · 표현)<br>` +
      `⛰️ <b>토</b> — 가운데서 붙잡는 힘 (환절기 · 신용 · 포용)<br>` +
      `⚔️ <b>금</b> — 거두고 자르는 힘 (가을 · 결단 · 원칙)<br>` +
      `💧 <b>수</b> — 모여서 스미는 힘 (겨울 · 지혜 · 저장)<br>` +
      (() => {
        const ents = Object.entries(r.ohaeng).sort((a, b) => b[1] - a[1]);
        const top = ents[0], zero = ents.filter(([, n]) => n === 0).map(([o]) => o);
        return `당신의 저울에서 가장 무거운 기운은 <b>${top[0]}(${top[1]}개)</b> — 삶의 기본 동작이 그쪽 문법이라는 뜻입니다. ` +
          (zero.length
            ? `없는 기운(<b>${zero.join("·")}</b>)은 결핍이 아니라 <b>덜 쓰는 채널</b>입니다 — 대운·세운이 그 기운을 들고 오는 시기에 수업이 시작됩니다.`
            : `다섯 기운이 모두 있어, 어떤 운이 와도 받아낼 채널이 있는 형입니다.`);
      })()],
    ["여기, 이 글자가 바로 '당신'입니다",
    ...
    ["십성은 다섯 그룹 — 삶의 순환 고리입니다",
      `<b>비겁</b>(나와 같은 기운 · 자아) → <b>식상</b>(내가 낳는 기운 · 표현) → <b>재성</b>(내가 다루는 기운 · 성과) → <b>관성</b>(나를 다잡는 기운 · 책임) → <b>인성</b>(나를 채우는 기운 · 배움) → 다시 비겁.<br>` +
      `내가 서고 → 표현하고 → 거두고 → 책임지고 → 배워서 다시 서는, <b>한 사람의 활동 사이클 전체</b>가 이 고리 안에 있습니다. 대운·세운 읽기란 결국 "지금 이 사이클의 어느 국면인가"를 보는 일입니다.<br>` +
      `한쪽에 몰리면 <b>전문가형 엔진</b>(스포츠카 — 특정 도로에서 압도적), 골고루면 <b>다재다능형</b>(SUV — 어디든 무난). 좋고 나쁨이 아니라 <b>차종의 차이</b>입니다. 없는 그룹도 결핍이 아니라 덜 쓰는 채널이고, 운에서 그 기운이 올 때가 그 영역의 수업 기간입니다.`],
    ["당신의 기질은 이렇습니다",
...
$("#foot").innerHTML =
  ...
  "의학적·법률적·재무적 판단의 근거로 사용될 수 없으며, 개인의 능력·가치·미래를 확정하지 않습니다.<br>" +
  '<a href="policy.html" style="color:inherit;text-decoration:underline">이용약관 · 개인정보처리방침</a>';
